...
module.exports = {
    ...
    ai: {
        ...
        system: ([ // 系统提示词
            ...
            "- 回复会自动以换行分割发送，如遇到不需要分割的内容（如代码、举例）请在开头与结尾用'```'包裹",
            "",
            "## 工具调用条件",
            "- 当该次对话时间与上次对话时间差异过大或处于对话开头时，必须先获取最近聊天记录，了解上下文后再回复",
            "- 当有拿不准的知识时，首先搜索知识库，如知识库无法查询，再调用联网搜索",
            "",
            "## 输入消息格式：",
            "[{时间}][{名字}({QQ})] >> {说的话}",
